...
if __name__ == "__main__":
    ...
    npz_filename = 'sinusoidal_new_pathCV_results.npz'
    if os.path.exists(npz_filename):
        ...
        path_nodes_list = [2, 4, 8, 16, 32, 64]#[2, 4, 6, 8, 16]
        Path_M = np.zeros(len(path_nodes_list))
        Path_E = np.zeros(len(path_nodes_list))

    # 2. Load Committor & Generate Pool
    grid_path = '/home/sorbonne/ProductionEntropy/Spotential/sol_ongrid.dat'
    ...
    save_filename = 'rss_committor_sinus_results.npz'
    print(f"\nSaving RSS results to '{save_filename}'...")
    
    #np.savez_compressed(
    #    save_filename,
    #    angles_deg=path_nodes_list,
    #    rss_list=rss_list,
    #)
